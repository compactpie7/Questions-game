import { useState } from "react";
import { useQuestions } from "../hooks/useQuestions";
import { useUpdateQuestion } from "../hooks/useUpdateQuestion";
import EditModal from "../components/EditModal/EditModal";
import "./questions.css";
import { useDeleteQuestion } from "../hooks/useDeleteQuestion";

export default function Questions() {
    const { questions, loading, error, refetch } = useQuestions();
    const { updateQuestion } = useUpdateQuestion();

    const [editingQuestion, setEditingQuestion] = useState<null | {
        id: number;
        text: string;
        passed: boolean;
    }>(null);


    const handleEdit = async (updated: { text: string; passed: boolean }) => {
        if (!editingQuestion) return;
        await updateQuestion(editingQuestion.id, updated);
        setEditingQuestion(null);
        refetch(); // reload the list
    };

    const { deleteQuestion } = useDeleteQuestion();

    const handleDelete = async (id: number) => {
        try {
            await deleteQuestion(id);
            refetch();
            setEditingQuestion(null); // This closes the modal by clearing the editing question
        } catch (error) {
            console.error(error);
            alert("Failed to delete question");
        }
    };


    if (loading) return <p>Loading questions...</p>;
    if (error) return <p>Error loading questions: {error.message}</p>;

    return (
        <div className="questions">
            <h1>Questions</h1>
            <div className="questions__list-wrapper">
                <ul className="questions__list">
                    {questions.map((q: any, index: any) => (
                        <li key={q.id} className={`list__item ${q.passed ? "passed" : ""}`}>
                            <span>{index + 1}.</span>
                            <span className="item__span">
                                {q.text} — {q.passed ? "✅" : "❌"}
                            </span>
                            <button className="item__button" onClick={() => setEditingQuestion(q)}>
                                <svg xmlns="http://www.w3.org/2000/svg" width="1rem" height="1rem" viewBox="0 0 24 24" fill="none">
                                    <path d="M11 4H7.2C6.0799 4 5.51984 4 5.09202 4.21799C4.71569 4.40974 4.40973 4.7157 4.21799 5.09202C4 5.51985 4 6.0799 4 7.2V16.8C4 17.9201 4 18.4802 4.21799 18.908C4.40973 19.2843 4.71569 19.5903 5.09202 19.782C5.51984 20 6.0799 20 7.2 20H16.8C17.9201 20 18.4802 20 18.908 19.782C19.2843 19.5903 19.5903 19.2843 19.782 18.908C20 18.4802 20 17.9201 20 16.8V12.5M15.5 5.5L18.3284 8.32843M10.7627 10.2373L17.411 3.58902C18.192 2.80797 19.4584 2.80797 20.2394 3.58902C21.0205 4.37007 21.0205 5.6364 20.2394 6.41745L13.3774 13.2794C12.6158 14.0411 12.235 14.4219 11.8012 14.7247C11.4162 14.9936 11.0009 15.2162 10.564 15.3882C10.0717 15.582 9.54378 15.6885 8.48793 15.9016L8 16L8.04745 15.6678C8.21536 14.4925 8.29932 13.9048 8.49029 13.3561C8.65975 12.8692 8.89125 12.4063 9.17906 11.9786C9.50341 11.4966 9.92319 11.0768 10.7627 10.2373Z" stroke="#DAAD86" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                                </svg>
                            </button>
                        </li>
                    ))}
                </ul>
            </div>

            {editingQuestion && (
                <EditModal
                    question={editingQuestion}
                    onClose={() => setEditingQuestion(null)}
                    onSave={handleEdit}
                    onDelete={handleDelete}
                />
            )}
        </div>
    );
}
